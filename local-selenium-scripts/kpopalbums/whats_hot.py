...
try:
    base_page_url = "https://www.kpopalbums.com/collections/whats-hot"
...
for page_ind in range(1, total_pages + 1):
    try:
        ...
        base_url = "https://www.kpopalbums.com/collections/whats-hot"
        ...
        if len(item_list) == 0:
            ...
            if "9891 items" in page_text or "items" in page_text.lower():
                msg = (f"Page {page_ind}: Page loaded but no items "
                       "found - may be empty or structure changed")
                logger.warning(msg)
            continue
...
if lists_equal_len and len(product_name_list) > 0:
    ...
    output_file = 'kpopalbums_whats_hot.csv'
