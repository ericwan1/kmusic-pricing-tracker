...
logger.info(f"Starting scrape for adult-contemporary (max {max_pages} pages)")

while not_an_empty_page and index < max_pages:
    try:
        logger.info(f"Scraping page {index}")
        base_url = "https://www.musicplaza.com/collections/adult-contemporary"
...
if lists_equal_len and len(product_name_list) > 0:
    ...
    output_file = 'musicplaza_adult_contemporary.csv'
