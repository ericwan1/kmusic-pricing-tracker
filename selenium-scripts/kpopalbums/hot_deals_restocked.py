...
# Do hot deals First
try:
    pg_count_url = "https://www.kpopalbums.com/collections/hot-deals?page=1&view=ajax"
    ...
    look_at_sites = False
    print("ERROR: could not grab hot deals page count from kpopalbums.com")
    pass

if look_at_sites:
    for page_ind in range(1,total_pages+1):
        try:
            pg_count_url = f"https://www.kpopalbums.com/collections/hot-deals?page={page_ind}&view=ajax"
            data = driver.get(pg_count_url)
            ...
            print("ERROR: scraping hot deals failed unexpectedly")
            fail_count += 1
            if fail_count < 10:
...
fail_count = 0
try:
    pg_count_url = "https://www.kpopalbums.com/collections/restocked?page=1&view=ajax"
    data = driver.get(pg_count_url)
    time.sleep(3)
    ...
    look_at_sites = False
    print("ERROR: could not grab restocked page count from kpopalbums.com")
    pass

if look_at_sites:
    for page_ind in range(1,total_pages+1):
        try:
            pg_count_url = f"https://www.kpopalbums.com/collections/restocked?page={page_ind}&view=ajax"
            data = driver.get(pg_count_url)
            time.sleep(random.randint(4,8))
            ...
            print("ERROR: scraping restocked failed unexpectedly")
            fail_count += 1
            if fail_count < 10:
                time.sleep(random.randint(30,60))
                continue
...
output_df.to_csv('kpopalbums_deals_restocked.csv')