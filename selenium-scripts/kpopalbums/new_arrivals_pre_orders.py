...
# Do Latest Releases First
try:
    pg_count_url = "https://www.kpopalbums.com/collections/lastest-release?page=1&view=ajax"
    ...
    look_at_sites = False
    print("ERROR: could not grab latest release page count from kpopalbums.com")
    pass
...
fail_count = 0
try:
    pg_count_url = "https://www.kpopalbums.com/collections/pre-order?page=1&view=ajax"
    data = driver.get(pg_count_url)
    time.sleep(3)
...
if look_at_sites:
    for page_ind in range(1,total_pages+1):
        try:
            ...
            print("ERROR: scraping pre-orders failed unexpectedly")
            fail_count += 1
            if fail_count < 10:
                time.sleep(random.randint(30,60))
                continue
...
output_df.to_csv('kpopalbums_latest_release_pre_orders.csv')